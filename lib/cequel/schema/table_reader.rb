    ...
    class TableReader
      ...
      def initialize(table_data)
        ...
        @table = Table.new(table_data.name,
                           table_data.is_a?(Cassandra::MaterializedView))
      end
      ...
      @@prop_extractors = [] # rubocop:disable Style/ClassVars
      def self.def_property(name,
                            option_method = name,
                            coercion = ->(val, _table_data) { val })
        @@prop_extractors << lambda { |table, table_data|
      ...
      def_property("compaction", :compaction_strategy,
                   lambda { |compaction_strategy, _table_data|
      ...
      attr_reader :table_data, :indexes
