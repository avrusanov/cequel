...
module Cequel
  module Schema
    ...
    class TableDiffer
      ...
      def added_indexes
        # ignore still indexed columns
        table_b.columns
               .select(&:indexed?)
               .reject do |c| 
          table_a.has_column?(c.name) &&
                    table_a.column(c.name).indexed?
        end
      end
      ...
      def dropped_indexes
        table_a.columns
               .select(&:indexed?)
               .select { |c| table_b.has_column?(c.name) } # ignore "dropped" columns
