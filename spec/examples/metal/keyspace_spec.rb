...
describe Cequel::Metal::Keyspace do
  ...
  describe '::batch' do
    it 'sends enclosed write statements in bulk' do
    ...
    it 'auto-applies if option given' do
    ...
    it 'does nothing if no statements executed in batch' do
      expect { cequel.batch {} }.not_to raise_error
    end

    it 'executes unlogged batch if specified' do
    ...
    it 'executes batch with given consistency' do
    ...
    it 'raises error if consistency specified in individual query in batch' do
      expect do
        cequel.batch(consistency: :one) do
  ...
  describe "#exists?" do
    it "is true for existent keyspaces", retry: 1, retry_wait: 1 do
      expect(cequel.exists?).to be true
    end
  ...
  describe 'cassandra error handling' do
    ...
    it 'rejects a negative value for retry delay' do
      expect do 
        Cequel.connect connect_options.merge(
          retry_delay: -1.0
        )
      end.to raise_error(ArgumentError)
    end
    ...
    it 'can clear active connections' do
      ...
      end.to(change(default_connect, :client))
    end
  end
