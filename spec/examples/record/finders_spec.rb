...
describe Cequel::Record::Finders do
  ...
  context 'simple primary key' do
    ...
    describe '#find_by_*' do
      ...
      it 'returns nil if no record matches' do
        expect(Blog.find_by_subdomain('bogus')).to be_nil
      end

      it 'responds to method before it is called' do
        expect(User).to respond_to(:find_by_login)
      end

      it 'raises error on wrong name' do
        expect { Blog.find_by_bogus('bogus') }.to raise_error(NoMethodError)
      end

      it 'does not respond to wrong name' do
        expect(User).not_to respond_to(:find_by_bogus)
      end
    end
    ...
    describe '#find_all_by_*' do
      ...
      it 'does not respond' do
        expect(User).not_to respond_to(:find_all_by_login)
      end
    end
  end
  ...
  context 'compound primary key' do
    ...
    describe '#find_all_by_*' do
      it 'returns all records matching key prefix' do
        expect(Post.find_all_by_blog_subdomain('cassandra'))
          .to eq(cassandra_posts)
      end

      it 'greedilies load records' do
      ...
      it 'does not exist for all keys' do
    ...
    describe '#find_by_*' do
      it 'returns record matching all keys' do
        expect(Post.find_by_blog_subdomain_and_id(
                 'cassandra', cassandra_posts.first.id
               )).to eq(cassandra_posts.first)
      end

      it 'casts arguments to correct type' do
        expect(Post.find_by_blog_subdomain_and_id(
      ...
      it 'does not exist for key prefix' do
        expect { Post.find_by_blog_subdomain('foo') }
          .to raise_error(NoMethodError)
      end

      it 'allows lower-order key if chained' do
  ...
  context 'secondary index' do
    ...
    it 'exposes scope to query by secondary index' do
      expect(Post.with_author_id(author_ids.first))
        .to match_array(cassandra_posts.values_at(0, 2, 4))
    end

    it 'exposes method to retrieve first result by secondary index' do
      expect(Post.find_by_author_id(author_ids.first))
        .to eq(cassandra_posts.first)
    end

    it 'exposes method to eagerly retrieve all results by secondary index' do
      posts = Post.find_all_by_author_id(author_ids.first)
      disallow_queries!
      expect(posts).to match_array(cassandra_posts.values_at(0, 2, 4))
    end

    it 'does not expose methods for non-indexed columns' do
      %i[find_by_title find_all_by_title with_title].each do |method|
        expect(Post).not_to respond_to(method)
      end
    end
  end
end
